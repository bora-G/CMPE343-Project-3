...
import com.group05.greengrocer.service.CouponService;
import com.group05.greengrocer.service.LoyaltyService;
import com.group05.greengrocer.service.OrderService;
...
public class CartController implements Initializable {
    ...
    private void updateTotal() {
        ...
        BigDecimal discountPercent = com.group05.greengrocer.service.LoyaltyService.getLoyaltyDiscountPercent();
        if (completedOrdersCount >= threshold) {
            loyaltyDiscount = subtotal.multiply(discountPercent).divide(new BigDecimal("100"), 2,
        ...
        couponDiscountLabel.setText(couponDiscount.compareTo(BigDecimal.ZERO) > 0
                ? "-₺" + couponDiscount.setScale(2, java.math.RoundingMode.HALF_UP)
                : "₺0.00");
        loyaltyDiscountLabel.setText(loyaltyDiscount.compareTo(BigDecimal.ZERO) > 0
                ? "-₺" + loyaltyDiscount.setScale(2, java.math.RoundingMode.HALF_UP)
                : "₺0.00");
    ...
    private void handleApplyCoupon() {
        ...
        int customerId = com.group05.greengrocer.util.Session.getInstance().getCurrentUserId();
    ...
    private void handleCheckout() {
        ...
        order.setCustomerId(com.group05.greengrocer.util.Session.getInstance().getCurrentUserId());
    ...
    private boolean showOrderSummary(String address, LocalDateTime deliveryDateTime) {
        ...
        if (loyaltyDiscount.compareTo(BigDecimal.ZERO) > 0) {
            summary.append("\nLoyalty Discount (5%): -₺")
                    .append(loyaltyDiscount.setScale(2, java.math.RoundingMode.HALF_UP));
        }
